...
def _templates() -> str:
    return (
        ...
        "## Typed metadata (optional)\n"
        "A template can declare what the **frontmatter** of its pages should look "
        "like — one property per line in the template editor's *Metadata schema* "
        "box:\n\n"
        "```\nhp: str\nblade*: int\nrole: player | npc\nborn: date\n```\n\n"
        "`*` after the name means required (the same marker custom elements use). "
        "Types: `str`, `int`, `float`, `bool`, `date`, `list[str]`, or a choice "
        "list. Pages created from the template are stamped with a `template:` "
        "property and checked against the schema on their **Metadata tab**.\n\n"
        "Checking **warns, it never blocks**: your value is always saved exactly "
        "as typed and never rewritten — a mismatch is reported, not corrected. "
        "Pages created before you wrote the schema carry no stamp and are left "
        "alone; add `template: <name>` to one by hand to opt it in.\n\n"
        "## Managing templates\n"
        "**Settings → Templates** lists them; each has its own edit page (name + "
        "body + optional metadata schema). Deleting a template never touches pages "
        "created from it — a page that names a template that no longer exists is "
        "simply unchecked.\n\n"
        "## For Claude (MCP)\n"
        "- `list_templates` — see available templates.\n"
        "- `create_template(name, markdown, meta_schema)` — add or replace one; "
        "omit `meta_schema` to leave an existing schema alone.\n"
        "- `create_from_template(name, title)` — make a page from a template.\n"
        "- `get_metadata` / `set_metadata` — the `schema` they return says whether "
        "a page matches its template (`errors`, plus `values` coerced to type).\n"
    )
...
def _sharing() -> str:
    return (
        ...
        "## Finding it without typing an address\n"
        "While sharing is on, Waikiki announces itself over **Bonjour**, so it "
        "turns up automatically on Macs and iPhones on the same network — "
        "Safari's Bonjour bookmarks, or `dns-sd -B _http._tcp` in a terminal. "
        "Guests can pick it from a list instead of typing an IP.\n\n"
        "## Who can do what\n"
        ...
        "**Settings → Share with someone remote** opens a temporary public "
        "`https://…trycloudflare.com` link (needs `cloudflared`). Send the link "
        "and the password separately — text the link, say the password out "
        "loud. Visitors get the same guest access, over HTTPS, and the link "
        "dies when you stop it or quit Waikiki.\n\n"
        "Be deliberate with it: unlike network sharing, anyone on the internet "
        "who has the link can reach the sign-in page. Keep it short-lived and "
